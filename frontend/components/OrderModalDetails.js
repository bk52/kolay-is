...
const OrderDetails=({order, onValueChanged,SaveOrder,prepaymentChanged})=>{
    return(
        ...
        <Grid item xs={6}><CurrencyTextField disabled label="Toplam Fiyat" fullWidth currencySymbol="₺" outputFormat="string" decimalPlaces={2} value={order.orderBalance.net} /></Grid>
        <Grid item xs={6}><TextField id="notes" name="notes" label="Sipariş Notu" value={order.note} onChange={(e)=>{onValueChanged("note",e.target.value)}} fullWidth/></Grid>
        <Grid item xs={4}>
        ...
        <Grid item xs={2} style={{marginTop:10,textAlign:"center"}}>
